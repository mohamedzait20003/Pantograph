...
/**
 * The app as a factory, separate from the process entry point in `index.ts`.
 *
 * Tests need to bind this to an ephemeral port in-process rather than shelling
 * out to a dev server: spawning one means guessing how long it takes to boot,
 * and a fixed port collides the moment two suites run at once. Building the app
 * here and listening there keeps importing it free of side effects.
 */
...
export function createApp(options: { tenant?: string } = {}): express.Express {
  ...
  app.get('/health', (_req, res) => {
    res.json({ ok: true, tenant, brand: brand.name });
  });
